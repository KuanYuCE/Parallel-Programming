...
int main(int argc, char *argv[])
{
  ...
  else if (PPLogger.getTotalInstrs() == 0)
  {
    printf("Not using fake intrinsics in ClampedExp\n");
  }
  else
  {
  ...
  //   // std::cout<< "sumGold: "  << sumGold << ", sumOutput: " << sumOutput << endl;
  ...
  //   else if (PPLogger.getTotalInstrs() == 0)
  //   {
  //     printf("Not using fake intrinsics in ArraySum\n");
  //   }
  //   else
  //   {
...
void initValue(float *values, int *exponents, float *output, float *gold, unsigned int N)
{
  ...
  for (unsigned int i = 0; i < N + VECTOR_WIDTH; i++)
  {
    ...
    values[i] =  4.f * static_cast<float>(rand()) / RAND_MAX;
...
bool verifyResult(float *values, int *exponents, float *output, float *gold, int N)
{
  ...
  for (int i = 0; i < N + VECTOR_WIDTH; i++)
  {
    ...
    {
      // std::cout << "output[i]: " << output[i] << ", gold[i]: " << gold[i] << endl;
      incorrect = i;
      break;
    }
  }
